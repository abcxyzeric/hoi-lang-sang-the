...
(function () {
    ...
    function isDaMingWorldView(variables) {
        const worldview = normalizeWorldViewLabel(getCurrentWorldView(variables));
        return worldview === 'đại minh chí dị' || worldview === 'dai minh chi di';
    }
    ...
    const STARTER_TEAMMATE_TEMPLATE_MAP = {
        ...
        'Asuna': {
            ...
            Chủng_tộc: 'Nhân_loại',
            Cấp_độ: 1,
            Thuộc_tính: { Sức_mạnh: 12, Nhanh_nhẹn: 16, Thể_chất: 12, Trí_lực: 12, Cảm_nhận: 16, Sức_hấp_dẫn: 18 },
            Danh_sách_trang_bị: {
                'Kiếm Phong Hoa': {
        ...
        'Lumioer': {
            ...
            Chủng_tộc: 'Long_tộc',
            Cấp_độ: 85,
            Thuộc_tính: { Sức_mạnh: 20, Nhanh_nhẹn: 26, Thể_chất: 24, Trí_lực: 22, Cảm_nhận: 16, Sức_hấp_dẫn: 26 },
            Danh_sách_trang_bị: {
                'Uy Tín Vàng Của Xích Dực': {
                    Loại: 'Vũ_khí', Vị_trí_trang_bị: 'Tay_chính', Tên: 'Uy Tín Vàng Của Xích Dực', Phẩm_chất: 'Sử_thi', Cấp_độ: 85, Cấp_cường_hóa: 0,
                    Cộng_thêm_thuộc_tính: { Sức_hấp_dẫn: 2, Thể_chất: 1, Toàn_kỹ_năng: 2, Sát_thương_bạo_kích: 40 },
                    Hiệu_quả: 'Kiểm định tấn công +2;[Kiêu Hãnh Bỏ Vảy] Khi Lumioer chủ động bỏ trạng thái tăng ích của bản thân hoặc khấu trừ 10% sinh mệnh hiện tại, có thể đổi bất kỳ đòn tấn công nào thành tấn công phạm vi, đồng thời khiến mục tiêu trúng đòn nhận [Uy Áp]: kiểm định tấn công kế tiếp -2;[Vang Lên, Ánh Vàng] Hành động phụ, mỗi trận 1 lần, Lumioer tuyên cáo tên Uy Tín Vàng, chọn một trong ba hiệu ứng sau để kích hoạt tới hết trận, không thể đổi: "Xích Dực Phá Trận" mỗi lượt chắc chắn tiên công, kiểm định tấn công +3; "Kim Dực Lung Hộ" giảm sát thương vật lý/ma pháp của bản thân +15%, đầu mỗi hiệp hồi sinh mệnh bằng 10% HP tối đa của Lumioer cho đồng minh gần nhất; "Đơn Dực Khảm Kim" ngưỡng bạo kích -3, sau mỗi lần bạo kích thì đòn tấn công kế tiếp chắc chắn trúng;[Long Tức Gầm Vang] Hành động phụ, mỗi ngày 1 lần, xóa toàn bộ trạng thái tăng ích của bản thân, kích nổ long mạch cuồng bạo vàng đỏ đan xen, gây [1000% + 25% × (Cấp_độ-1)] sát thương AOE thuộc tính lửa lên mọi kẻ địch trong tầm nhìn.',
                    Mô_tả: 'Nguyên là một đoạn kết tinh vảy rồng đỏ thẫm tróc xuống khi trưởng nữ long tộc đơn dực chào đời, về sau được nàng mài thành chiến nhận tượng trưng thân phận. Ngoại hình như lưỡi chém thon dài đan giữa đỏ thẫm và ám kim, sống kiếm chảy ánh hồng chuyển sắc.',
                    Hòm_trang_bị: false
                }
            },
            Kỹ_năng: {
                'Gầm Vang Đi, Ánh Vàng': { Phẩm_chất: 'Sử_thi', Loại: 'Sát_thương', Hiệu_quả: 'Bày ra tư thái "Uy Tín Vàng", cuốn bão long tức vàng đỏ quét sạch toàn trường. Bắt buộc tiêu hao 10% HP hoặc 1 trạng thái tăng ích để phát động, gây [1000% + 25% × (Cấp_độ - 1)] sát thương thuộc tính lửa lên toàn bộ kẻ địch. Mỗi lần phóng thích kỹ năng này chỉ tính một lần; nếu kỹ năng này hạ sát bất kỳ kẻ địch nào, Lumioer được hành động thêm một lần.', Mô_tả: 'Ánh vàng vang lên, khắc tên ta vào chiến trường.' },
                'Một Kích Lấp Lánh Của Đơn Dực': { Phẩm_chất: 'Truyền_thuyết', Loại: 'Sát_thương', Hiệu_quả: 'Mở đơn dực đỏ thẫm trong suốt bay lên, thực hiện một đòn tia sáng từ trên cao đánh xuống. Gây [800% + 20% × (Cấp_độ - 1)] sát thương thuộc tính ánh sáng lên mọi kẻ địch trong phạm vi. Sau khi trúng, cấp cho toàn bộ đồng minh khiên bằng 25% HP tối đa của Lumioer và xóa trạng thái giảm ích trên đồng minh.', Mô_tả: 'Dù chỉ có một cánh, nàng vẫn có thể nhẹ nhàng nhảy lên không trung, để nắng xuyên qua màng cánh đỏ thẫm và rải ánh ấm xuống mọi người được nàng che chở.' }
            },
            Ngoại_hình: 'Long nương xích dực tóc vàng xoăn dài, đuôi tóc chuyển dần sang hồng đậm, mắt vàng hổ phách, dáng trẻ con nhỏ nhắn đến mức thấp nhất nhà. Lưng phải sinh một đơn dực đỏ thẫm trong suốt, trên đầu có sừng rồng cành đỏ, đuôi rồng chuyển sắc vàng đỏ thường cuộn bên người; khi cảm xúc dao động, chóp đuôi sẽ vô thức lắc lư.',
            Trang_phục: 'Bên ngoài áo liền thân bó sát màu trắng phủ giáp ngực đen khảm hồng ngọc, viền vàng phối hoa văn thoi đen, vai khoác áo choàng ren trắng nhỏ, găng tay dài đen in vân vảy rồng hình thoi, chân đi bốt cao gót vàng trang trí thủy tinh.',
            Độ_thiện_cảm: 35,
            Lời_thề_đồng_hành: true,
            Tuyệt_kỹ_liên_kết: {
                'Xích Dực Cùng Diệu Kim Khiêu Vũ': { Phẩm_chất: 'Sử_thi', Loại: 'Sát_thương', Hiệu_quả: 'Tiêu hao 100 CP. Người chơi ôm Lumioer nhỏ nhắn từ sau lưng, ma lực hóa thành một cánh ánh sáng khác áp vào đơn dực đỏ thẫm của nàng để bù trọn. Hai người hợp lực kích nổ năng lượng long mạch áp đảo, gây [1200% + 35% × (Cấp_độ-1)] sát thương hỗn hợp lửa/ánh sáng lên toàn bộ kẻ địch. Đòn này chắc chắn trúng, đồng thời khiến toàn bộ kháng tính của phe địch giảm 20% tới hết trận. Sau khi phóng thích, Lumioer miễn nhiễm hiệu ứng khống chế trong 3 lượt.', Mô_tả: 'Nhìn cho kỹ, đây chính là ánh sáng vượt khỏi lẽ thường!' }
            }
        }
    };
    ...
    function handleExperienceProcessing(rawVariables, rawVariablesBefore) {
        ...
        try {
            const keyAdapter = window.HLSTKeyAdapter;
            const statData = keyAdapter ? keyAdapter.toInternal(rawVariables?.stat_data) : rawVariables?.stat_data;
            const statDataBefore = keyAdapter ? keyAdapter.toInternal(rawVariablesBefore?.stat_data) : rawVariablesBefore?.stat_data;
            ...
            // Độ thành thạo tự động thăng bậc và rollback bậc do AI sửa
            handleProficiency(statData, statDataBefore);

            // Đồng hồ tận thế, chỉ chạy trong thế giới quan Đại Minh chí dị
            ...
            if (keyAdapter && rawVariables && statData) {
                rawVariables.stat_data = keyAdapter.toCanonical(statData);
            }
        } finally {
            isProcessing = false;
        }
    ...
    // ==========================================
    // Độ thành thạo tự động thăng bậc và rollback AI
    // ==========================================

    const PROF_TIERS = ['Học_việc', 'Thạo_việc', 'Chuyên_gia', 'Đại_sư', 'Truyền_kỳ'];
    const PROF_THRESHOLDS = { 'Học_việc': 25, 'Thạo_việc': 50, 'Chuyên_gia': 100, 'Đại_sư': 200, 'Truyền_kỳ': Infinity };
    const PROF_TIER_BONUS = { 'Học_việc': 0, 'Thạo_việc': 1, 'Chuyên_gia': 2, 'Đại_sư': 4, 'Truyền_kỳ': 6 };
        ...
        Object.entries(block).forEach(([key, entry]) => {
            ...
            if (!oldEntry) {
                ...
                if (safeParseInt(entry.Ngưỡng_thăng_bậc, 0) !== correctThreshold) {
                    console.warn(`[Độ thành thạo bảo vệ] ⚠️ ${label}/${key} Ngưỡng_thăng_bậc lúc đăng ký bị lỗi(${entry.Ngưỡng_thăng_bậc}), đã sửa thành ${correctThreshold}`);
                    entry.Ngưỡng_thăng_bậc = correctThreshold;
                }
                entry.Hiệu_quả = getProfEffect(key, tier);
                console.log(`[Độ thành thạo đăng ký] ${label}/${key}: Bậc=${tier}, ngưỡng=${correctThreshold}, Hiệu_quả="${entry.Hiệu_quả}"`);
            }

            if (oldEntry && oldEntry.Bậc && entry.Bậc !== oldEntry.Bậc) {
                console.warn(`[Độ thành thạo bảo vệ] ⚠️ ${label}/${key} Bậc bị bên ngoài sửa ${oldEntry.Bậc}→${entry.Bậc}, đã rollback`);
                entry.Bậc = oldEntry.Bậc;
                entry.Ngưỡng_thăng_bậc = oldEntry.Ngưỡng_thăng_bậc;
                entry.Hiệu_quả = oldEntry.Hiệu_quả || getProfEffect(key, oldEntry.Bậc);
            }

            if (oldEntry && oldEntry.Hiệu_quả && entry.Hiệu_quả !== oldEntry.Hiệu_quả) {
                console.warn(`[Độ thành thạo bảo vệ] ⚠️ ${label}/${key} Hiệu_quả bị bên ngoài sửa "${oldEntry.Hiệu_quả}"→"${entry.Hiệu_quả}", đã rollback`);
            ...
            const correctThreshold = PROF_THRESHOLDS[tier] === Infinity ? 0 : (PROF_THRESHOLDS[tier] || 100);
            if (safeParseInt(entry.Ngưỡng_thăng_bậc, 0) !== correctThreshold) {
                console.warn(`[Độ thành thạo bảo vệ] ⚠️ ${label}/${key} Ngưỡng_thăng_bậc lỗi(${entry.Ngưỡng_thăng_bậc}), đã sửa thành ${correctThreshold}`);
                entry.Ngưỡng_thăng_bậc = correctThreshold;
            }
            ...
            if (progress >= correctThreshold) {
                const nextTier = getNextProfTier(tier);
                if (nextTier) {
                    entry.Bậc = nextTier;
                    entry.Tiến_độ = progress - correctThreshold;
                    ...
                    console.log(`[Độ thành thạo thăng bậc] ${label}/${key}: ${tier} → ${nextTier}, tiến độ dư=${entry.Tiến_độ}, Hiệu_quả="${entry.Hiệu_quả}"`);
                }
            }

            if (!entry.Hiệu_quả) {
                entry.Hiệu_quả = getProfEffect(key, entry.Bậc || 'Học_việc');
            }
        });
    }
    ...
    // ==========================================
    // Đồng hồ tận thế, chỉ trong thế giới quan Đại Minh chí dị
    // Mô hình: tăng theo ngày năm 1/2/3 = 1/2/4, nhân với số đời Lục Sinh Ngũ Thế còn sống / 5.
    // Mỗi công tích AI thêm vào Công_tích_đã_trấn_áp sẽ trừ giả ngẫu nhiên 1-3 điểm; đủ 1000 là hoàn chỉnh thể giáng lâm sớm.
    // Nếu chống qua 3 năm mà chưa đủ 1000, giáng lâm ở trạng thái không hoàn chỉnh.
    // ==========================================
    const DOOM_RAMP = [1, 2, 4];
    const DOOM_YEAR_DAYS = 365;
    const DOOM_DEADLINE_DAYS = 1095;
    const DOOM_MAX = 1000;
    const DOOM_HISTORY_CAP = 10;
    const DOOM_KILL_FLAT = 150;
    const LIUSHENG_NAMES = ['Tuế_Viễn', 'Hoang', 'Trác_Chiếu', 'Phan_Nam_Quân', 'Bách_Lý_Uyên'];
    ...
    function handleDoomClock(statData, statDataBefore) {
        if (!isDaMingWorldView(statData)) return;
        let clock = statData.Đồng_hồ_tận_thế;
        if (!clock || typeof clock !== 'object') {
            clock = {};
            statData.Đồng_hồ_tận_thế = clock;
        }

        const worldCalendar = statData?.Thông_tin_thế_giới?.Niên_lịch;
        if (!clock.Niên_lịch_bắt_đầu || !parseCalendarDate(clock.Niên_lịch_bắt_đầu)) {
            if (parseCalendarDate(worldCalendar)) clock.Niên_lịch_bắt_đầu = worldCalendar;
        }
        const startParsed = parseCalendarDate(clock.Niên_lịch_bắt_đầu);
        const nowParsed = parseCalendarDate(worldCalendar);
        if (!startParsed || !nowParsed) {
            console.warn(`[Đồng hồ tận thế] Niên lịch không thể phân tích (bắt đầu=${clock.Niên_lịch_bắt_đầu}, hiện tại=${worldCalendar}), bỏ qua`);
            return;
        }
        let elapsedDays = getCalendarDayDiff(nowParsed.date, startParsed.date);
        if (elapsedDays < 0) elapsedDays = 0;

        if (!clock.Đã_thanh_trừ || typeof clock.Đã_thanh_trừ !== 'object') clock.Đã_thanh_trừ = {};
        if (!clock.Ngày_thanh_trừ || typeof clock.Ngày_thanh_trừ !== 'object') clock.Ngày_thanh_trừ = {};
        LIUSHENG_NAMES.forEach(name => {
            if (typeof clock.Đã_thanh_trừ[name] !== 'boolean') clock.Đã_thanh_trừ[name] = false;
            if (clock.Đã_thanh_trừ[name] === true && typeof clock.Ngày_thanh_trừ[name] !== 'number') {
                clock.Ngày_thanh_trừ[name] = elapsedDays;
                console.log(`[Đồng hồ tận thế] đã thanh trừ "${name}" ở ngày ${elapsedDays}, tốc độ tăng sau đó giảm một bậc`);
            } else if (clock.Đã_thanh_trừ[name] !== true && typeof clock.Ngày_thanh_trừ[name] === 'number') {
                delete clock.Ngày_thanh_trừ[name];
            }
        });

        if (!Array.isArray(clock.Công_tích_đã_trấn_áp)) clock.Công_tích_đã_trấn_áp = [];
        const prevArr = Array.isArray(statDataBefore?.Đồng_hồ_tận_thế?.Công_tích_đã_trấn_áp)
            ? statDataBefore.Đồng_hồ_tận_thế.Công_tích_đã_trấn_áp : [];
        let newCount = clock.Công_tích_đã_trấn_áp.length - prevArr.length;
        if (newCount < 0) newCount = 0;
        if (newCount > 5) newCount = 5;
        if (newCount > 0) {
            clock.Số_lần_công_tích = safeParseInt(clock.Số_lần_công_tích, 0) + newCount;
        }
        if (clock.Công_tích_đã_trấn_áp.length > DOOM_HISTORY_CAP) {
            clock.Công_tích_đã_trấn_áp = clock.Công_tích_đã_trấn_áp.slice(clock.Công_tích_đã_trấn_áp.length - DOOM_HISTORY_CAP);
        }

        let gongyeReduce = 0;
        const seedBase = String(clock.Niên_lịch_bắt_đầu || 'Đồng_hồ_tận_thế');
        const gongyeCount = safeParseInt(clock.Số_lần_công_tích, 0);
        for (let i = 1; i <= gongyeCount; i++) {
            gongyeReduce += 1 + (stableHash(seedBase + ':công_tích:' + i) % 3);
        }

        const killDays = LIUSHENG_NAMES
            .map(n => clock.Ngày_thanh_trừ?.[n])
        ...
        clock.Điểm_hiện_tại = points;
        clock.Số_đời_còn_sống = 5 - killDays.length;

        if (points >= DOOM_MAX) {
            clock.Trạng_thái = 'Hoàn_chỉnh_thể_giáng_lâm';
        } else if (elapsedDays >= DOOM_DEADLINE_DAYS) {
            clock.Trạng_thái = 'Không_hoàn_chỉnh_thể_giáng_lâm';
        } else {
            clock.Trạng_thái = 'Đang_ủ_mầm';
        }
    }
