(function () {
    ...
    const SCRIPT_NAME = 'Script dọn thẻ nội dung tầng';
    const LOADED_FLAG = '__script_don_the_noi_dung_tang_loaded__';
    const GLOBAL_API_NAME = '__fixMessageTagBlocks__';
    // Chỉ sửa tại đây: tương thích thẻ <think> mới nhất, bảo đảm tách đúng vùng suy nghĩ và định vị phần trước <now_plot>.
    ...
    function normalizeTagBlocks(message) {
        ...
        // Giữ nguyên logic tại đây: dùng hasBeautifyCardOutsideNowPlot để phát hiện card nằm trước <now_plot>.
        const shouldRewrapNowPlot = hasNowPlotPair && hasBeautifyCardOutsideNowPlot(middleWithoutAction);
        
        // Giữ lõi xử lý: khi phát hiện card bên ngoài, loại thẻ cũ ở đầu/cuối rồi để `normalized += <now_plot>` và `</now_plot>` bên dưới bọc lại thống nhất.
        ...
        if (hasNowPlotPair && !shouldRewrapNowPlot) {
            ...
            // Tuyệt đối giữ nguyên xử lý thẻ cuối.
            normalized += `</now_plot>`;
        }
    ...
    async function rewriteMessage(messageId, chatMessage, normalizedMessage, options = {}) {
        ...
        if (!targetMessage) {
            throw new Error(`[${SCRIPT_NAME}] Không tìm thấy tầng ${messageId} có thể ghi lại trong context.chat`);
        }
        ...
        const logMessage = `[${SCRIPT_NAME}] Đã ghi lại tầng ${messageId}`;
        if (logWrite) {
    ...
    async function fixMessageTagsById(messageId, options = {}) {
        ...
        if (!Number.isInteger(messageId) || messageId < 0) {
            const error = new Error(`[${SCRIPT_NAME}] Số tầng không hợp lệ: ${messageId}`);
            if (notify) {
        ...
        try {
            ...
            if (!chatMessage) {
                ...
                if (notify) {
                    toastr.warning(`[${SCRIPT_NAME}] Không tìm thấy tầng ${messageId}`);
                }
                return result;
            }
            ...
            if (!shouldWrite) {
                if (notify) {
                    const tip = normalizedResult.skipped
                        ? `[${SCRIPT_NAME}] Tầng ${messageId} chưa xử lý: ${normalizedResult.reason}`
                        : `[${SCRIPT_NAME}] Tầng ${messageId} không cần sửa`;
                    toastr.info(tip);
                }
                return {
                    ...normalizedResult,
            ...
            if (notify) {
                toastr.error(`[${SCRIPT_NAME}] Xử lý tầng ${messageId} thất bại: ${error?.message || String(error)}`);
            }
            throw error;
        } finally {
    ...
    function unload() {
        while (listenerStops.length > 0) {
            ...
            try {
                ...
                console.warn(`[${SCRIPT_NAME}] Gỡ bộ lắng nghe thất bại`, error);
            }
        }
        ...
        console.log(`[${SCRIPT_NAME}] Đã gỡ tải`);
    }
    ...
    function resetMount() {
        ...
        console.log(`[${SCRIPT_NAME}] Đã đặt lại điểm gắn`);
    }

    const init = async () => {
        const root = getRootWindow();
        if (root[LOADED_FLAG]) {
            console.log(`[${SCRIPT_NAME}] Phát hiện phiên bản cũ, gỡ trước rồi tải lại`);
            try {
                ...
                console.warn(`[${SCRIPT_NAME}] Gỡ phiên bản cũ thất bại, tiếp tục thử tải lại`, error);
                root[LOADED_FLAG] = false;
            }
        }
        ...
        console.log(`[${SCRIPT_NAME}] Script đã tải`);
        console.log(`[${SCRIPT_NAME}] Kích hoạt thủ công: ${GLOBAL_API_NAME}.fixLatestMessageTags({ notify: true })`);
        if (typeof toastr !== 'undefined') {
            toastr.success(`[${SCRIPT_NAME}] Script đã tải`);
        }
    };
